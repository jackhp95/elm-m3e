module M3e.Cem.Html.ListAction exposing
    ( listAction, disabled, download, href, rel, target
    , onClick
    )

{-|
Bottom layer for `<m3e-list-action>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, R1-correct DOM emission, no phantom typing. The rawest escape in the gradient.
