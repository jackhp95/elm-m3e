module M3e.Cem.Html.ListItemButton exposing
    ( listItemButton, href, target, rel, download, disabled
    , onClick
    )

{-|
Bottom layer for `<m3e-list-item-button>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, R1-correct DOM emission, no phantom typing. The rawest escape in the gradient.
