module M3e.Build.BottomSheet.Slots exposing
    ( headerTree, headerTreeItem, headerToolbar, headerToc, headerTocItem, headerThemeIcon
    , headerTheme, headerTextareaAutosize, headerTabs, headerTabPanel, headerTab, headerSwitch, headerStepperReset
    , headerStepperPrevious, headerStep, headerStepPanel, headerStepper, headerSplitPane, headerSplitButton, headerSnackbar
    , headerSlider, headerSliderThumb, headerSlideGroup, headerSkeleton, headerShape, headerSegmentedButton, headerButtonSegment
    , headerSearchView, headerSearchBar, headerRadioGroup, headerRadio, headerProgressElementIndicatorBase, headerPaginator, headerSelect
    , headerNavRailToggle, headerNavRail, headerNavMenuItemGroup, headerNavMenu, headerNavMenuItem, headerNavBar, headerNavItem
    , headerMenuItemRadio, headerMenuItemGroup, headerMenuItemCheckbox, headerMenu, headerMenuItem, headerMenuTrigger, headerMenuItemElementBase
    , headerLoadingIndicator, headerSelectionList, headerListOption, headerActionList, headerExpandableListItem, headerListAction, headerListItemButton
    , headerList, headerListItem, headerIcon, headerHeading, headerFabMenuTrigger, headerFabMenu, headerFab
    , headerAccordion, headerExpansionPanel, headerExpansionHeader, headerDrawerToggle, headerDrawerContainer, headerDivider, headerDialogTrigger
    , headerDialog, headerDialogAction, headerDatepickerToggle, headerDatepicker, headerContentPane, headerSuggestionChip, headerInputChipSet
    , headerInputChip, headerFilterChipSet, headerFilterChip, headerChipSet, headerAssistChip, headerChip, headerCheckbox
    , headerCard, headerCalendar, headerYearView, headerMultiYearView, headerMonthView, headerTooltip, headerRichTooltip
    , headerTooltipElementBase, headerRichTooltipAction, headerButtonGroup, headerIconButton, headerButton, headerBreadcrumb, headerBreadcrumbItem
    , headerBreadcrumbItemButton, headerBottomSheetTrigger, headerBottomSheet, headerBottomSheetAction, headerBadge, headerAvatar, headerAutocomplete
    , headerFormField, headerOptionPanel, headerFloatingPanel, headerOptgroup, headerOption, headerFocusTrap, headerAppBar
    , headerTextOverflow, headerTextHighlight, headerStateLayer, headerSlide, headerScrollContainer, headerRipple, headerPseudoRadio
    , headerPseudoCheckbox, headerFocusRing, headerElevation, headerCollapsible, headerActionElementBase, tree, treeItem
    , toolbar, toc, tocItem, themeIcon, theme, textareaAutosize, tabs
    , tabPanel, tab, switch, stepperReset, stepperPrevious, step, stepPanel
    , stepper, splitPane, splitButton, snackbar, slider, sliderThumb, slideGroup
    , skeleton, shape, segmentedButton, buttonSegment, searchView, searchBar, radioGroup
    , radio, progressElementIndicatorBase, paginator, select, navRailToggle, navRail, navMenuItemGroup
    , navMenu, navMenuItem, navBar, navItem, menuItemRadio, menuItemGroup, menuItemCheckbox
    , menu, menuItem, menuTrigger, menuItemElementBase, loadingIndicator, selectionList, listOption
    , actionList, expandableListItem, listAction, listItemButton, list, listItem, icon
    , heading, fabMenuTrigger, fabMenu, fab, accordion, expansionPanel, expansionHeader
    , drawerToggle, drawerContainer, divider, dialogTrigger, dialog, dialogAction, datepickerToggle
    , datepicker, contentPane, suggestionChip, inputChipSet, inputChip, filterChipSet, filterChip
    , chipSet, assistChip, chip, checkbox, card, calendar, yearView
    , multiYearView, monthView, tooltip, richTooltip, tooltipElementBase, richTooltipAction, buttonGroup
    , iconButton, button, breadcrumb, breadcrumbItem, breadcrumbItemButton, bottomSheetTrigger, bottomSheet
    , bottomSheetAction, badge, avatar, autocomplete, formField, optionPanel, floatingPanel
    , optgroup, option, focusTrap, appBar, textOverflow, textHighlight, stateLayer
    , slide, scrollContainer, ripple, pseudoRadio, pseudoCheckbox, focusRing, elevation
    , collapsible, actionElementBase
    )

{-|
Slot setters for `M3e.Build.BottomSheet`. Each alias accepts a specific child component's Builder and inserts it into the parent's slot.

@docs headerTree, headerTreeItem, headerToolbar, headerToc, headerTocItem, headerThemeIcon
@docs headerTheme, headerTextareaAutosize, headerTabs, headerTabPanel, headerTab, headerSwitch
@docs headerStepperReset, headerStepperPrevious, headerStep, headerStepPanel, headerStepper, headerSplitPane
@docs headerSplitButton, headerSnackbar, headerSlider, headerSliderThumb, headerSlideGroup, headerSkeleton
@docs headerShape, headerSegmentedButton, headerButtonSegment, headerSearchView, headerSearchBar, headerRadioGroup
@docs headerRadio, headerProgressElementIndicatorBase, headerPaginator, headerSelect, headerNavRailToggle, headerNavRail
@docs headerNavMenuItemGroup, headerNavMenu, headerNavMenuItem, headerNavBar, headerNavItem, headerMenuItemRadio
@docs headerMenuItemGroup, headerMenuItemCheckbox, headerMenu, headerMenuItem, headerMenuTrigger, headerMenuItemElementBase
@docs headerLoadingIndicator, headerSelectionList, headerListOption, headerActionList, headerExpandableListItem, headerListAction
@docs headerListItemButton, headerList, headerListItem, headerIcon, headerHeading, headerFabMenuTrigger
@docs headerFabMenu, headerFab, headerAccordion, headerExpansionPanel, headerExpansionHeader, headerDrawerToggle
@docs headerDrawerContainer, headerDivider, headerDialogTrigger, headerDialog, headerDialogAction, headerDatepickerToggle
@docs headerDatepicker, headerContentPane, headerSuggestionChip, headerInputChipSet, headerInputChip, headerFilterChipSet
@docs headerFilterChip, headerChipSet, headerAssistChip, headerChip, headerCheckbox, headerCard
@docs headerCalendar, headerYearView, headerMultiYearView, headerMonthView, headerTooltip, headerRichTooltip
@docs headerTooltipElementBase, headerRichTooltipAction, headerButtonGroup, headerIconButton, headerButton, headerBreadcrumb
@docs headerBreadcrumbItem, headerBreadcrumbItemButton, headerBottomSheetTrigger, headerBottomSheet, headerBottomSheetAction, headerBadge
@docs headerAvatar, headerAutocomplete, headerFormField, headerOptionPanel, headerFloatingPanel, headerOptgroup
@docs headerOption, headerFocusTrap, headerAppBar, headerTextOverflow, headerTextHighlight, headerStateLayer
@docs headerSlide, headerScrollContainer, headerRipple, headerPseudoRadio, headerPseudoCheckbox, headerFocusRing
@docs headerElevation, headerCollapsible, headerActionElementBase, tree, treeItem, toolbar
@docs toc, tocItem, themeIcon, theme, textareaAutosize, tabs
@docs tabPanel, tab, switch, stepperReset, stepperPrevious, step
@docs stepPanel, stepper, splitPane, splitButton, snackbar, slider
@docs sliderThumb, slideGroup, skeleton, shape, segmentedButton, buttonSegment
@docs searchView, searchBar, radioGroup, radio, progressElementIndicatorBase, paginator
@docs select, navRailToggle, navRail, navMenuItemGroup, navMenu, navMenuItem
@docs navBar, navItem, menuItemRadio, menuItemGroup, menuItemCheckbox, menu
@docs menuItem, menuTrigger, menuItemElementBase, loadingIndicator, selectionList, listOption
@docs actionList, expandableListItem, listAction, listItemButton, list, listItem
@docs icon, heading, fabMenuTrigger, fabMenu, fab, accordion
@docs expansionPanel, expansionHeader, drawerToggle, drawerContainer, divider, dialogTrigger
@docs dialog, dialogAction, datepickerToggle, datepicker, contentPane, suggestionChip
@docs inputChipSet, inputChip, filterChipSet, filterChip, chipSet, assistChip
@docs chip, checkbox, card, calendar, yearView, multiYearView
@docs monthView, tooltip, richTooltip, tooltipElementBase, richTooltipAction, buttonGroup
@docs iconButton, button, breadcrumb, breadcrumbItem, breadcrumbItemButton, bottomSheetTrigger
@docs bottomSheet, bottomSheetAction, badge, avatar, autocomplete, formField
@docs optionPanel, floatingPanel, optgroup, option, focusTrap, appBar
@docs textOverflow, textHighlight, stateLayer, slide, scrollContainer, ripple
@docs pseudoRadio, pseudoCheckbox, focusRing, elevation, collapsible, actionElementBase
-}


import M3e.Build.Accordion
import M3e.Build.ActionElementBase
import M3e.Build.ActionList
import M3e.Build.AppBar
import M3e.Build.AssistChip
import M3e.Build.Autocomplete
import M3e.Build.Avatar
import M3e.Build.Badge
import M3e.Build.BottomSheet
import M3e.Build.BottomSheetAction
import M3e.Build.BottomSheetTrigger
import M3e.Build.Breadcrumb
import M3e.Build.BreadcrumbItem
import M3e.Build.BreadcrumbItemButton
import M3e.Build.Button
import M3e.Build.ButtonGroup
import M3e.Build.ButtonSegment
import M3e.Build.Calendar
import M3e.Build.Card
import M3e.Build.Checkbox
import M3e.Build.Chip
import M3e.Build.ChipSet
import M3e.Build.Collapsible
import M3e.Build.ContentPane
import M3e.Build.Datepicker
import M3e.Build.DatepickerToggle
import M3e.Build.Dialog
import M3e.Build.DialogAction
import M3e.Build.DialogTrigger
import M3e.Build.Divider
import M3e.Build.DrawerContainer
import M3e.Build.DrawerToggle
import M3e.Build.Elevation
import M3e.Build.ExpandableListItem
import M3e.Build.ExpansionHeader
import M3e.Build.ExpansionPanel
import M3e.Build.Fab
import M3e.Build.FabMenu
import M3e.Build.FabMenuTrigger
import M3e.Build.FilterChip
import M3e.Build.FilterChipSet
import M3e.Build.FloatingPanel
import M3e.Build.FocusRing
import M3e.Build.FocusTrap
import M3e.Build.FormField
import M3e.Build.Heading
import M3e.Build.Icon
import M3e.Build.IconButton
import M3e.Build.InputChip
import M3e.Build.InputChipSet
import M3e.Build.Internal
import M3e.Build.List
import M3e.Build.ListAction
import M3e.Build.ListItem
import M3e.Build.ListItemButton
import M3e.Build.ListOption
import M3e.Build.LoadingIndicator
import M3e.Build.Menu
import M3e.Build.MenuItem
import M3e.Build.MenuItemCheckbox
import M3e.Build.MenuItemElementBase
import M3e.Build.MenuItemGroup
import M3e.Build.MenuItemRadio
import M3e.Build.MenuTrigger
import M3e.Build.MonthView
import M3e.Build.MultiYearView
import M3e.Build.NavBar
import M3e.Build.NavItem
import M3e.Build.NavMenu
import M3e.Build.NavMenuItem
import M3e.Build.NavMenuItemGroup
import M3e.Build.NavRail
import M3e.Build.NavRailToggle
import M3e.Build.Optgroup
import M3e.Build.Option
import M3e.Build.OptionPanel
import M3e.Build.Paginator
import M3e.Build.ProgressElementIndicatorBase
import M3e.Build.PseudoCheckbox
import M3e.Build.PseudoRadio
import M3e.Build.Radio
import M3e.Build.RadioGroup
import M3e.Build.RichTooltip
import M3e.Build.RichTooltipAction
import M3e.Build.Ripple
import M3e.Build.ScrollContainer
import M3e.Build.SearchBar
import M3e.Build.SearchView
import M3e.Build.SegmentedButton
import M3e.Build.Select
import M3e.Build.SelectionList
import M3e.Build.Shape
import M3e.Build.Skeleton
import M3e.Build.Slide
import M3e.Build.SlideGroup
import M3e.Build.Slider
import M3e.Build.SliderThumb
import M3e.Build.Snackbar
import M3e.Build.SplitButton
import M3e.Build.SplitPane
import M3e.Build.StateLayer
import M3e.Build.Step
import M3e.Build.StepPanel
import M3e.Build.Stepper
import M3e.Build.StepperPrevious
import M3e.Build.StepperReset
import M3e.Build.SuggestionChip
import M3e.Build.Switch
import M3e.Build.Tab
import M3e.Build.TabPanel
import M3e.Build.Tabs
import M3e.Build.TextHighlight
import M3e.Build.TextOverflow
import M3e.Build.TextareaAutosize
import M3e.Build.Theme
import M3e.Build.ThemeIcon
import M3e.Build.Toc
import M3e.Build.TocItem
import M3e.Build.Toolbar
import M3e.Build.Tooltip
import M3e.Build.TooltipElementBase
import M3e.Build.Tree
import M3e.Build.TreeItem
import M3e.Build.YearView
import M3e.Node


header_core :
    M3e.Build.Internal.Builder ck ca cs msg
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
header_core child_ parent_ =
    M3e.Build.Internal.wrap_
        (M3e.Node.addChild
             (M3e.Build.Internal.node_ child_)
             (M3e.Build.Internal.node_ parent_)
        )


unnamed_core :
    M3e.Build.Internal.Builder anyK anyA anyS msg
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
unnamed_core child_ parent_ =
    M3e.Build.Internal.wrap_
        (M3e.Node.addChild
             (M3e.Build.Internal.node_ child_)
             (M3e.Build.Internal.node_ parent_)
        )


{-| Place a `Tree` in the `header` slot of `BottomSheet`. -}
headerTree :
    M3e.Build.Tree.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTree =
    header_core


{-| Place a `TreeItem` in the `header` slot of `BottomSheet`. -}
headerTreeItem :
    M3e.Build.TreeItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTreeItem =
    header_core


{-| Place a `Toolbar` in the `header` slot of `BottomSheet`. -}
headerToolbar :
    M3e.Build.Toolbar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerToolbar =
    header_core


{-| Place a `Toc` in the `header` slot of `BottomSheet`. -}
headerToc :
    M3e.Build.Toc.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerToc =
    header_core


{-| Place a `TocItem` in the `header` slot of `BottomSheet`. -}
headerTocItem :
    M3e.Build.TocItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTocItem =
    header_core


{-| Place a `ThemeIcon` in the `header` slot of `BottomSheet`. -}
headerThemeIcon :
    M3e.Build.ThemeIcon.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerThemeIcon =
    header_core


{-| Place a `Theme` in the `header` slot of `BottomSheet`. -}
headerTheme :
    M3e.Build.Theme.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTheme =
    header_core


{-| Place a `TextareaAutosize` in the `header` slot of `BottomSheet`. -}
headerTextareaAutosize :
    M3e.Build.TextareaAutosize.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTextareaAutosize =
    header_core


{-| Place a `Tabs` in the `header` slot of `BottomSheet`. -}
headerTabs :
    M3e.Build.Tabs.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTabs =
    header_core


{-| Place a `TabPanel` in the `header` slot of `BottomSheet`. -}
headerTabPanel :
    M3e.Build.TabPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTabPanel =
    header_core


{-| Place a `Tab` in the `header` slot of `BottomSheet`. -}
headerTab :
    M3e.Build.Tab.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTab =
    header_core


{-| Place a `Switch` in the `header` slot of `BottomSheet`. -}
headerSwitch :
    M3e.Build.Switch.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSwitch =
    header_core


{-| Place a `StepperReset` in the `header` slot of `BottomSheet`. -}
headerStepperReset :
    M3e.Build.StepperReset.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStepperReset =
    header_core


{-| Place a `StepperPrevious` in the `header` slot of `BottomSheet`. -}
headerStepperPrevious :
    M3e.Build.StepperPrevious.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStepperPrevious =
    header_core


{-| Place a `Step` in the `header` slot of `BottomSheet`. -}
headerStep :
    M3e.Build.Step.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStep =
    header_core


{-| Place a `StepPanel` in the `header` slot of `BottomSheet`. -}
headerStepPanel :
    M3e.Build.StepPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStepPanel =
    header_core


{-| Place a `Stepper` in the `header` slot of `BottomSheet`. -}
headerStepper :
    M3e.Build.Stepper.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStepper =
    header_core


{-| Place a `SplitPane` in the `header` slot of `BottomSheet`. -}
headerSplitPane :
    M3e.Build.SplitPane.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSplitPane =
    header_core


{-| Place a `SplitButton` in the `header` slot of `BottomSheet`. -}
headerSplitButton :
    M3e.Build.SplitButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSplitButton =
    header_core


{-| Place a `Snackbar` in the `header` slot of `BottomSheet`. -}
headerSnackbar :
    M3e.Build.Snackbar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSnackbar =
    header_core


{-| Place a `Slider` in the `header` slot of `BottomSheet`. -}
headerSlider :
    M3e.Build.Slider.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSlider =
    header_core


{-| Place a `SliderThumb` in the `header` slot of `BottomSheet`. -}
headerSliderThumb :
    M3e.Build.SliderThumb.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSliderThumb =
    header_core


{-| Place a `SlideGroup` in the `header` slot of `BottomSheet`. -}
headerSlideGroup :
    M3e.Build.SlideGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSlideGroup =
    header_core


{-| Place a `Skeleton` in the `header` slot of `BottomSheet`. -}
headerSkeleton :
    M3e.Build.Skeleton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSkeleton =
    header_core


{-| Place a `Shape` in the `header` slot of `BottomSheet`. -}
headerShape :
    M3e.Build.Shape.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerShape =
    header_core


{-| Place a `SegmentedButton` in the `header` slot of `BottomSheet`. -}
headerSegmentedButton :
    M3e.Build.SegmentedButton.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSegmentedButton =
    header_core


{-| Place a `ButtonSegment` in the `header` slot of `BottomSheet`. -}
headerButtonSegment :
    M3e.Build.ButtonSegment.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerButtonSegment =
    header_core


{-| Place a `SearchView` in the `header` slot of `BottomSheet`. -}
headerSearchView :
    M3e.Build.SearchView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSearchView =
    header_core


{-| Place a `SearchBar` in the `header` slot of `BottomSheet`. -}
headerSearchBar :
    M3e.Build.SearchBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSearchBar =
    header_core


{-| Place a `RadioGroup` in the `header` slot of `BottomSheet`. -}
headerRadioGroup :
    M3e.Build.RadioGroup.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerRadioGroup =
    header_core


{-| Place a `Radio` in the `header` slot of `BottomSheet`. -}
headerRadio :
    M3e.Build.Radio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerRadio =
    header_core


{-| Place a `ProgressElementIndicatorBase` in the `header` slot of `BottomSheet`. -}
headerProgressElementIndicatorBase :
    M3e.Build.ProgressElementIndicatorBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerProgressElementIndicatorBase =
    header_core


{-| Place a `Paginator` in the `header` slot of `BottomSheet`. -}
headerPaginator :
    M3e.Build.Paginator.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerPaginator =
    header_core


{-| Place a `Select` in the `header` slot of `BottomSheet`. -}
headerSelect :
    M3e.Build.Select.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSelect =
    header_core


{-| Place a `NavRailToggle` in the `header` slot of `BottomSheet`. -}
headerNavRailToggle :
    M3e.Build.NavRailToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavRailToggle =
    header_core


{-| Place a `NavRail` in the `header` slot of `BottomSheet`. -}
headerNavRail :
    M3e.Build.NavRail.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavRail =
    header_core


{-| Place a `NavMenuItemGroup` in the `header` slot of `BottomSheet`. -}
headerNavMenuItemGroup :
    M3e.Build.NavMenuItemGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavMenuItemGroup =
    header_core


{-| Place a `NavMenu` in the `header` slot of `BottomSheet`. -}
headerNavMenu :
    M3e.Build.NavMenu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavMenu =
    header_core


{-| Place a `NavMenuItem` in the `header` slot of `BottomSheet`. -}
headerNavMenuItem :
    M3e.Build.NavMenuItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavMenuItem =
    header_core


{-| Place a `NavBar` in the `header` slot of `BottomSheet`. -}
headerNavBar :
    M3e.Build.NavBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavBar =
    header_core


{-| Place a `NavItem` in the `header` slot of `BottomSheet`. -}
headerNavItem :
    M3e.Build.NavItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerNavItem =
    header_core


{-| Place a `MenuItemRadio` in the `header` slot of `BottomSheet`. -}
headerMenuItemRadio :
    M3e.Build.MenuItemRadio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuItemRadio =
    header_core


{-| Place a `MenuItemGroup` in the `header` slot of `BottomSheet`. -}
headerMenuItemGroup :
    M3e.Build.MenuItemGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuItemGroup =
    header_core


{-| Place a `MenuItemCheckbox` in the `header` slot of `BottomSheet`. -}
headerMenuItemCheckbox :
    M3e.Build.MenuItemCheckbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuItemCheckbox =
    header_core


{-| Place a `Menu` in the `header` slot of `BottomSheet`. -}
headerMenu :
    M3e.Build.Menu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenu =
    header_core


{-| Place a `MenuItem` in the `header` slot of `BottomSheet`. -}
headerMenuItem :
    M3e.Build.MenuItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuItem =
    header_core


{-| Place a `MenuTrigger` in the `header` slot of `BottomSheet`. -}
headerMenuTrigger :
    M3e.Build.MenuTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuTrigger =
    header_core


{-| Place a `MenuItemElementBase` in the `header` slot of `BottomSheet`. -}
headerMenuItemElementBase :
    M3e.Build.MenuItemElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMenuItemElementBase =
    header_core


{-| Place a `LoadingIndicator` in the `header` slot of `BottomSheet`. -}
headerLoadingIndicator :
    M3e.Build.LoadingIndicator.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerLoadingIndicator =
    header_core


{-| Place a `SelectionList` in the `header` slot of `BottomSheet`. -}
headerSelectionList :
    M3e.Build.SelectionList.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSelectionList =
    header_core


{-| Place a `ListOption` in the `header` slot of `BottomSheet`. -}
headerListOption :
    M3e.Build.ListOption.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerListOption =
    header_core


{-| Place a `ActionList` in the `header` slot of `BottomSheet`. -}
headerActionList :
    M3e.Build.ActionList.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerActionList =
    header_core


{-| Place a `ExpandableListItem` in the `header` slot of `BottomSheet`. -}
headerExpandableListItem :
    M3e.Build.ExpandableListItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerExpandableListItem =
    header_core


{-| Place a `ListAction` in the `header` slot of `BottomSheet`. -}
headerListAction :
    M3e.Build.ListAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerListAction =
    header_core


{-| Place a `ListItemButton` in the `header` slot of `BottomSheet`. -}
headerListItemButton :
    M3e.Build.ListItemButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerListItemButton =
    header_core


{-| Place a `List` in the `header` slot of `BottomSheet`. -}
headerList :
    M3e.Build.List.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerList =
    header_core


{-| Place a `ListItem` in the `header` slot of `BottomSheet`. -}
headerListItem :
    M3e.Build.ListItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerListItem =
    header_core


{-| Place a `Icon` in the `header` slot of `BottomSheet`. -}
headerIcon :
    M3e.Build.Icon.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerIcon =
    header_core


{-| Place a `Heading` in the `header` slot of `BottomSheet`. -}
headerHeading :
    M3e.Build.Heading.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerHeading =
    header_core


{-| Place a `FabMenuTrigger` in the `header` slot of `BottomSheet`. -}
headerFabMenuTrigger :
    M3e.Build.FabMenuTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFabMenuTrigger =
    header_core


{-| Place a `FabMenu` in the `header` slot of `BottomSheet`. -}
headerFabMenu :
    M3e.Build.FabMenu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFabMenu =
    header_core


{-| Place a `Fab` in the `header` slot of `BottomSheet`. -}
headerFab :
    M3e.Build.Fab.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFab =
    header_core


{-| Place a `Accordion` in the `header` slot of `BottomSheet`. -}
headerAccordion :
    M3e.Build.Accordion.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerAccordion =
    header_core


{-| Place a `ExpansionPanel` in the `header` slot of `BottomSheet`. -}
headerExpansionPanel :
    M3e.Build.ExpansionPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerExpansionPanel =
    header_core


{-| Place a `ExpansionHeader` in the `header` slot of `BottomSheet`. -}
headerExpansionHeader :
    M3e.Build.ExpansionHeader.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerExpansionHeader =
    header_core


{-| Place a `DrawerToggle` in the `header` slot of `BottomSheet`. -}
headerDrawerToggle :
    M3e.Build.DrawerToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDrawerToggle =
    header_core


{-| Place a `DrawerContainer` in the `header` slot of `BottomSheet`. -}
headerDrawerContainer :
    M3e.Build.DrawerContainer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDrawerContainer =
    header_core


{-| Place a `Divider` in the `header` slot of `BottomSheet`. -}
headerDivider :
    M3e.Build.Divider.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDivider =
    header_core


{-| Place a `DialogTrigger` in the `header` slot of `BottomSheet`. -}
headerDialogTrigger :
    M3e.Build.DialogTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDialogTrigger =
    header_core


{-| Place a `Dialog` in the `header` slot of `BottomSheet`. -}
headerDialog :
    M3e.Build.Dialog.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDialog =
    header_core


{-| Place a `DialogAction` in the `header` slot of `BottomSheet`. -}
headerDialogAction :
    M3e.Build.DialogAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDialogAction =
    header_core


{-| Place a `DatepickerToggle` in the `header` slot of `BottomSheet`. -}
headerDatepickerToggle :
    M3e.Build.DatepickerToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDatepickerToggle =
    header_core


{-| Place a `Datepicker` in the `header` slot of `BottomSheet`. -}
headerDatepicker :
    M3e.Build.Datepicker.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerDatepicker =
    header_core


{-| Place a `ContentPane` in the `header` slot of `BottomSheet`. -}
headerContentPane :
    M3e.Build.ContentPane.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerContentPane =
    header_core


{-| Place a `SuggestionChip` in the `header` slot of `BottomSheet`. -}
headerSuggestionChip :
    M3e.Build.SuggestionChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSuggestionChip =
    header_core


{-| Place a `InputChipSet` in the `header` slot of `BottomSheet`. -}
headerInputChipSet :
    M3e.Build.InputChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerInputChipSet =
    header_core


{-| Place a `InputChip` in the `header` slot of `BottomSheet`. -}
headerInputChip :
    M3e.Build.InputChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerInputChip =
    header_core


{-| Place a `FilterChipSet` in the `header` slot of `BottomSheet`. -}
headerFilterChipSet :
    M3e.Build.FilterChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFilterChipSet =
    header_core


{-| Place a `FilterChip` in the `header` slot of `BottomSheet`. -}
headerFilterChip :
    M3e.Build.FilterChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFilterChip =
    header_core


{-| Place a `ChipSet` in the `header` slot of `BottomSheet`. -}
headerChipSet :
    M3e.Build.ChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerChipSet =
    header_core


{-| Place a `AssistChip` in the `header` slot of `BottomSheet`. -}
headerAssistChip :
    M3e.Build.AssistChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerAssistChip =
    header_core


{-| Place a `Chip` in the `header` slot of `BottomSheet`. -}
headerChip :
    M3e.Build.Chip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerChip =
    header_core


{-| Place a `Checkbox` in the `header` slot of `BottomSheet`. -}
headerCheckbox :
    M3e.Build.Checkbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerCheckbox =
    header_core


{-| Place a `Card` in the `header` slot of `BottomSheet`. -}
headerCard :
    M3e.Build.Card.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerCard =
    header_core


{-| Place a `Calendar` in the `header` slot of `BottomSheet`. -}
headerCalendar :
    M3e.Build.Calendar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerCalendar =
    header_core


{-| Place a `YearView` in the `header` slot of `BottomSheet`. -}
headerYearView :
    M3e.Build.YearView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerYearView =
    header_core


{-| Place a `MultiYearView` in the `header` slot of `BottomSheet`. -}
headerMultiYearView :
    M3e.Build.MultiYearView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMultiYearView =
    header_core


{-| Place a `MonthView` in the `header` slot of `BottomSheet`. -}
headerMonthView :
    M3e.Build.MonthView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerMonthView =
    header_core


{-| Place a `Tooltip` in the `header` slot of `BottomSheet`. -}
headerTooltip :
    M3e.Build.Tooltip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTooltip =
    header_core


{-| Place a `RichTooltip` in the `header` slot of `BottomSheet`. -}
headerRichTooltip :
    M3e.Build.RichTooltip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerRichTooltip =
    header_core


{-| Place a `TooltipElementBase` in the `header` slot of `BottomSheet`. -}
headerTooltipElementBase :
    M3e.Build.TooltipElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTooltipElementBase =
    header_core


{-| Place a `RichTooltipAction` in the `header` slot of `BottomSheet`. -}
headerRichTooltipAction :
    M3e.Build.RichTooltipAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerRichTooltipAction =
    header_core


{-| Place a `ButtonGroup` in the `header` slot of `BottomSheet`. -}
headerButtonGroup :
    M3e.Build.ButtonGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerButtonGroup =
    header_core


{-| Place a `IconButton` in the `header` slot of `BottomSheet`. -}
headerIconButton :
    M3e.Build.IconButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerIconButton =
    header_core


{-| Place a `Button` in the `header` slot of `BottomSheet`. -}
headerButton :
    M3e.Build.Button.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerButton =
    header_core


{-| Place a `Breadcrumb` in the `header` slot of `BottomSheet`. -}
headerBreadcrumb :
    M3e.Build.Breadcrumb.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBreadcrumb =
    header_core


{-| Place a `BreadcrumbItem` in the `header` slot of `BottomSheet`. -}
headerBreadcrumbItem :
    M3e.Build.BreadcrumbItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBreadcrumbItem =
    header_core


{-| Place a `BreadcrumbItemButton` in the `header` slot of `BottomSheet`. -}
headerBreadcrumbItemButton :
    M3e.Build.BreadcrumbItemButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBreadcrumbItemButton =
    header_core


{-| Place a `BottomSheetTrigger` in the `header` slot of `BottomSheet`. -}
headerBottomSheetTrigger :
    M3e.Build.BottomSheetTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBottomSheetTrigger =
    header_core


{-| Place a `BottomSheet` in the `header` slot of `BottomSheet`. -}
headerBottomSheet :
    M3e.Build.BottomSheet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBottomSheet =
    header_core


{-| Place a `BottomSheetAction` in the `header` slot of `BottomSheet`. -}
headerBottomSheetAction :
    M3e.Build.BottomSheetAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBottomSheetAction =
    header_core


{-| Place a `Badge` in the `header` slot of `BottomSheet`. -}
headerBadge :
    M3e.Build.Badge.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerBadge =
    header_core


{-| Place a `Avatar` in the `header` slot of `BottomSheet`. -}
headerAvatar :
    M3e.Build.Avatar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerAvatar =
    header_core


{-| Place a `Autocomplete` in the `header` slot of `BottomSheet`. -}
headerAutocomplete :
    M3e.Build.Autocomplete.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerAutocomplete =
    header_core


{-| Place a `FormField` in the `header` slot of `BottomSheet`. -}
headerFormField :
    M3e.Build.FormField.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFormField =
    header_core


{-| Place a `OptionPanel` in the `header` slot of `BottomSheet`. -}
headerOptionPanel :
    M3e.Build.OptionPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerOptionPanel =
    header_core


{-| Place a `FloatingPanel` in the `header` slot of `BottomSheet`. -}
headerFloatingPanel :
    M3e.Build.FloatingPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFloatingPanel =
    header_core


{-| Place a `Optgroup` in the `header` slot of `BottomSheet`. -}
headerOptgroup :
    M3e.Build.Optgroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerOptgroup =
    header_core


{-| Place a `Option` in the `header` slot of `BottomSheet`. -}
headerOption :
    M3e.Build.Option.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerOption =
    header_core


{-| Place a `FocusTrap` in the `header` slot of `BottomSheet`. -}
headerFocusTrap :
    M3e.Build.FocusTrap.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFocusTrap =
    header_core


{-| Place a `AppBar` in the `header` slot of `BottomSheet`. -}
headerAppBar :
    M3e.Build.AppBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerAppBar =
    header_core


{-| Place a `TextOverflow` in the `header` slot of `BottomSheet`. -}
headerTextOverflow :
    M3e.Build.TextOverflow.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTextOverflow =
    header_core


{-| Place a `TextHighlight` in the `header` slot of `BottomSheet`. -}
headerTextHighlight :
    M3e.Build.TextHighlight.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerTextHighlight =
    header_core


{-| Place a `StateLayer` in the `header` slot of `BottomSheet`. -}
headerStateLayer :
    M3e.Build.StateLayer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerStateLayer =
    header_core


{-| Place a `Slide` in the `header` slot of `BottomSheet`. -}
headerSlide :
    M3e.Build.Slide.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerSlide =
    header_core


{-| Place a `ScrollContainer` in the `header` slot of `BottomSheet`. -}
headerScrollContainer :
    M3e.Build.ScrollContainer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerScrollContainer =
    header_core


{-| Place a `Ripple` in the `header` slot of `BottomSheet`. -}
headerRipple :
    M3e.Build.Ripple.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerRipple =
    header_core


{-| Place a `PseudoRadio` in the `header` slot of `BottomSheet`. -}
headerPseudoRadio :
    M3e.Build.PseudoRadio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerPseudoRadio =
    header_core


{-| Place a `PseudoCheckbox` in the `header` slot of `BottomSheet`. -}
headerPseudoCheckbox :
    M3e.Build.PseudoCheckbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerPseudoCheckbox =
    header_core


{-| Place a `FocusRing` in the `header` slot of `BottomSheet`. -}
headerFocusRing :
    M3e.Build.FocusRing.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerFocusRing =
    header_core


{-| Place a `Elevation` in the `header` slot of `BottomSheet`. -}
headerElevation :
    M3e.Build.Elevation.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerElevation =
    header_core


{-| Place a `Collapsible` in the `header` slot of `BottomSheet`. -}
headerCollapsible :
    M3e.Build.Collapsible.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerCollapsible =
    header_core


{-| Place a `ActionElementBase` in the `header` slot of `BottomSheet`. -}
headerActionElementBase :
    M3e.Build.ActionElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Available
    } msg pk
    -> M3e.Build.BottomSheet.Builder pa { ps
        | header : M3e.Build.Internal.Used
    } msg pk
headerActionElementBase =
    header_core


{-| Place a `Tree` in the `unnamed` slot of `BottomSheet`. -}
tree :
    M3e.Build.Tree.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tree =
    unnamed_core


{-| Place a `TreeItem` in the `unnamed` slot of `BottomSheet`. -}
treeItem :
    M3e.Build.TreeItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
treeItem =
    unnamed_core


{-| Place a `Toolbar` in the `unnamed` slot of `BottomSheet`. -}
toolbar :
    M3e.Build.Toolbar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
toolbar =
    unnamed_core


{-| Place a `Toc` in the `unnamed` slot of `BottomSheet`. -}
toc :
    M3e.Build.Toc.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
toc =
    unnamed_core


{-| Place a `TocItem` in the `unnamed` slot of `BottomSheet`. -}
tocItem :
    M3e.Build.TocItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tocItem =
    unnamed_core


{-| Place a `ThemeIcon` in the `unnamed` slot of `BottomSheet`. -}
themeIcon :
    M3e.Build.ThemeIcon.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
themeIcon =
    unnamed_core


{-| Place a `Theme` in the `unnamed` slot of `BottomSheet`. -}
theme :
    M3e.Build.Theme.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
theme =
    unnamed_core


{-| Place a `TextareaAutosize` in the `unnamed` slot of `BottomSheet`. -}
textareaAutosize :
    M3e.Build.TextareaAutosize.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
textareaAutosize =
    unnamed_core


{-| Place a `Tabs` in the `unnamed` slot of `BottomSheet`. -}
tabs :
    M3e.Build.Tabs.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tabs =
    unnamed_core


{-| Place a `TabPanel` in the `unnamed` slot of `BottomSheet`. -}
tabPanel :
    M3e.Build.TabPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tabPanel =
    unnamed_core


{-| Place a `Tab` in the `unnamed` slot of `BottomSheet`. -}
tab :
    M3e.Build.Tab.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tab =
    unnamed_core


{-| Place a `Switch` in the `unnamed` slot of `BottomSheet`. -}
switch :
    M3e.Build.Switch.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
switch =
    unnamed_core


{-| Place a `StepperReset` in the `unnamed` slot of `BottomSheet`. -}
stepperReset :
    M3e.Build.StepperReset.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
stepperReset =
    unnamed_core


{-| Place a `StepperPrevious` in the `unnamed` slot of `BottomSheet`. -}
stepperPrevious :
    M3e.Build.StepperPrevious.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
stepperPrevious =
    unnamed_core


{-| Place a `Step` in the `unnamed` slot of `BottomSheet`. -}
step :
    M3e.Build.Step.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
step =
    unnamed_core


{-| Place a `StepPanel` in the `unnamed` slot of `BottomSheet`. -}
stepPanel :
    M3e.Build.StepPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
stepPanel =
    unnamed_core


{-| Place a `Stepper` in the `unnamed` slot of `BottomSheet`. -}
stepper :
    M3e.Build.Stepper.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
stepper =
    unnamed_core


{-| Place a `SplitPane` in the `unnamed` slot of `BottomSheet`. -}
splitPane :
    M3e.Build.SplitPane.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
splitPane =
    unnamed_core


{-| Place a `SplitButton` in the `unnamed` slot of `BottomSheet`. -}
splitButton :
    M3e.Build.SplitButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
splitButton =
    unnamed_core


{-| Place a `Snackbar` in the `unnamed` slot of `BottomSheet`. -}
snackbar :
    M3e.Build.Snackbar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
snackbar =
    unnamed_core


{-| Place a `Slider` in the `unnamed` slot of `BottomSheet`. -}
slider :
    M3e.Build.Slider.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
slider =
    unnamed_core


{-| Place a `SliderThumb` in the `unnamed` slot of `BottomSheet`. -}
sliderThumb :
    M3e.Build.SliderThumb.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
sliderThumb =
    unnamed_core


{-| Place a `SlideGroup` in the `unnamed` slot of `BottomSheet`. -}
slideGroup :
    M3e.Build.SlideGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
slideGroup =
    unnamed_core


{-| Place a `Skeleton` in the `unnamed` slot of `BottomSheet`. -}
skeleton :
    M3e.Build.Skeleton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
skeleton =
    unnamed_core


{-| Place a `Shape` in the `unnamed` slot of `BottomSheet`. -}
shape :
    M3e.Build.Shape.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
shape =
    unnamed_core


{-| Place a `SegmentedButton` in the `unnamed` slot of `BottomSheet`. -}
segmentedButton :
    M3e.Build.SegmentedButton.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
segmentedButton =
    unnamed_core


{-| Place a `ButtonSegment` in the `unnamed` slot of `BottomSheet`. -}
buttonSegment :
    M3e.Build.ButtonSegment.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
buttonSegment =
    unnamed_core


{-| Place a `SearchView` in the `unnamed` slot of `BottomSheet`. -}
searchView :
    M3e.Build.SearchView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
searchView =
    unnamed_core


{-| Place a `SearchBar` in the `unnamed` slot of `BottomSheet`. -}
searchBar :
    M3e.Build.SearchBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
searchBar =
    unnamed_core


{-| Place a `RadioGroup` in the `unnamed` slot of `BottomSheet`. -}
radioGroup :
    M3e.Build.RadioGroup.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
radioGroup =
    unnamed_core


{-| Place a `Radio` in the `unnamed` slot of `BottomSheet`. -}
radio :
    M3e.Build.Radio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
radio =
    unnamed_core


{-| Place a `ProgressElementIndicatorBase` in the `unnamed` slot of `BottomSheet`. -}
progressElementIndicatorBase :
    M3e.Build.ProgressElementIndicatorBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
progressElementIndicatorBase =
    unnamed_core


{-| Place a `Paginator` in the `unnamed` slot of `BottomSheet`. -}
paginator :
    M3e.Build.Paginator.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
paginator =
    unnamed_core


{-| Place a `Select` in the `unnamed` slot of `BottomSheet`. -}
select :
    M3e.Build.Select.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
select =
    unnamed_core


{-| Place a `NavRailToggle` in the `unnamed` slot of `BottomSheet`. -}
navRailToggle :
    M3e.Build.NavRailToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navRailToggle =
    unnamed_core


{-| Place a `NavRail` in the `unnamed` slot of `BottomSheet`. -}
navRail :
    M3e.Build.NavRail.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navRail =
    unnamed_core


{-| Place a `NavMenuItemGroup` in the `unnamed` slot of `BottomSheet`. -}
navMenuItemGroup :
    M3e.Build.NavMenuItemGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navMenuItemGroup =
    unnamed_core


{-| Place a `NavMenu` in the `unnamed` slot of `BottomSheet`. -}
navMenu :
    M3e.Build.NavMenu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navMenu =
    unnamed_core


{-| Place a `NavMenuItem` in the `unnamed` slot of `BottomSheet`. -}
navMenuItem :
    M3e.Build.NavMenuItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navMenuItem =
    unnamed_core


{-| Place a `NavBar` in the `unnamed` slot of `BottomSheet`. -}
navBar :
    M3e.Build.NavBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navBar =
    unnamed_core


{-| Place a `NavItem` in the `unnamed` slot of `BottomSheet`. -}
navItem :
    M3e.Build.NavItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
navItem =
    unnamed_core


{-| Place a `MenuItemRadio` in the `unnamed` slot of `BottomSheet`. -}
menuItemRadio :
    M3e.Build.MenuItemRadio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuItemRadio =
    unnamed_core


{-| Place a `MenuItemGroup` in the `unnamed` slot of `BottomSheet`. -}
menuItemGroup :
    M3e.Build.MenuItemGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuItemGroup =
    unnamed_core


{-| Place a `MenuItemCheckbox` in the `unnamed` slot of `BottomSheet`. -}
menuItemCheckbox :
    M3e.Build.MenuItemCheckbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuItemCheckbox =
    unnamed_core


{-| Place a `Menu` in the `unnamed` slot of `BottomSheet`. -}
menu :
    M3e.Build.Menu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menu =
    unnamed_core


{-| Place a `MenuItem` in the `unnamed` slot of `BottomSheet`. -}
menuItem :
    M3e.Build.MenuItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuItem =
    unnamed_core


{-| Place a `MenuTrigger` in the `unnamed` slot of `BottomSheet`. -}
menuTrigger :
    M3e.Build.MenuTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuTrigger =
    unnamed_core


{-| Place a `MenuItemElementBase` in the `unnamed` slot of `BottomSheet`. -}
menuItemElementBase :
    M3e.Build.MenuItemElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
menuItemElementBase =
    unnamed_core


{-| Place a `LoadingIndicator` in the `unnamed` slot of `BottomSheet`. -}
loadingIndicator :
    M3e.Build.LoadingIndicator.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
loadingIndicator =
    unnamed_core


{-| Place a `SelectionList` in the `unnamed` slot of `BottomSheet`. -}
selectionList :
    M3e.Build.SelectionList.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
selectionList =
    unnamed_core


{-| Place a `ListOption` in the `unnamed` slot of `BottomSheet`. -}
listOption :
    M3e.Build.ListOption.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
listOption =
    unnamed_core


{-| Place a `ActionList` in the `unnamed` slot of `BottomSheet`. -}
actionList :
    M3e.Build.ActionList.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
actionList =
    unnamed_core


{-| Place a `ExpandableListItem` in the `unnamed` slot of `BottomSheet`. -}
expandableListItem :
    M3e.Build.ExpandableListItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
expandableListItem =
    unnamed_core


{-| Place a `ListAction` in the `unnamed` slot of `BottomSheet`. -}
listAction :
    M3e.Build.ListAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
listAction =
    unnamed_core


{-| Place a `ListItemButton` in the `unnamed` slot of `BottomSheet`. -}
listItemButton :
    M3e.Build.ListItemButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
listItemButton =
    unnamed_core


{-| Place a `List` in the `unnamed` slot of `BottomSheet`. -}
list :
    M3e.Build.List.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
list =
    unnamed_core


{-| Place a `ListItem` in the `unnamed` slot of `BottomSheet`. -}
listItem :
    M3e.Build.ListItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
listItem =
    unnamed_core


{-| Place a `Icon` in the `unnamed` slot of `BottomSheet`. -}
icon :
    M3e.Build.Icon.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
icon =
    unnamed_core


{-| Place a `Heading` in the `unnamed` slot of `BottomSheet`. -}
heading :
    M3e.Build.Heading.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
heading =
    unnamed_core


{-| Place a `FabMenuTrigger` in the `unnamed` slot of `BottomSheet`. -}
fabMenuTrigger :
    M3e.Build.FabMenuTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
fabMenuTrigger =
    unnamed_core


{-| Place a `FabMenu` in the `unnamed` slot of `BottomSheet`. -}
fabMenu :
    M3e.Build.FabMenu.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
fabMenu =
    unnamed_core


{-| Place a `Fab` in the `unnamed` slot of `BottomSheet`. -}
fab :
    M3e.Build.Fab.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
fab =
    unnamed_core


{-| Place a `Accordion` in the `unnamed` slot of `BottomSheet`. -}
accordion :
    M3e.Build.Accordion.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
accordion =
    unnamed_core


{-| Place a `ExpansionPanel` in the `unnamed` slot of `BottomSheet`. -}
expansionPanel :
    M3e.Build.ExpansionPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
expansionPanel =
    unnamed_core


{-| Place a `ExpansionHeader` in the `unnamed` slot of `BottomSheet`. -}
expansionHeader :
    M3e.Build.ExpansionHeader.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
expansionHeader =
    unnamed_core


{-| Place a `DrawerToggle` in the `unnamed` slot of `BottomSheet`. -}
drawerToggle :
    M3e.Build.DrawerToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
drawerToggle =
    unnamed_core


{-| Place a `DrawerContainer` in the `unnamed` slot of `BottomSheet`. -}
drawerContainer :
    M3e.Build.DrawerContainer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
drawerContainer =
    unnamed_core


{-| Place a `Divider` in the `unnamed` slot of `BottomSheet`. -}
divider :
    M3e.Build.Divider.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
divider =
    unnamed_core


{-| Place a `DialogTrigger` in the `unnamed` slot of `BottomSheet`. -}
dialogTrigger :
    M3e.Build.DialogTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
dialogTrigger =
    unnamed_core


{-| Place a `Dialog` in the `unnamed` slot of `BottomSheet`. -}
dialog :
    M3e.Build.Dialog.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
dialog =
    unnamed_core


{-| Place a `DialogAction` in the `unnamed` slot of `BottomSheet`. -}
dialogAction :
    M3e.Build.DialogAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
dialogAction =
    unnamed_core


{-| Place a `DatepickerToggle` in the `unnamed` slot of `BottomSheet`. -}
datepickerToggle :
    M3e.Build.DatepickerToggle.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
datepickerToggle =
    unnamed_core


{-| Place a `Datepicker` in the `unnamed` slot of `BottomSheet`. -}
datepicker :
    M3e.Build.Datepicker.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
datepicker =
    unnamed_core


{-| Place a `ContentPane` in the `unnamed` slot of `BottomSheet`. -}
contentPane :
    M3e.Build.ContentPane.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
contentPane =
    unnamed_core


{-| Place a `SuggestionChip` in the `unnamed` slot of `BottomSheet`. -}
suggestionChip :
    M3e.Build.SuggestionChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
suggestionChip =
    unnamed_core


{-| Place a `InputChipSet` in the `unnamed` slot of `BottomSheet`. -}
inputChipSet :
    M3e.Build.InputChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
inputChipSet =
    unnamed_core


{-| Place a `InputChip` in the `unnamed` slot of `BottomSheet`. -}
inputChip :
    M3e.Build.InputChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
inputChip =
    unnamed_core


{-| Place a `FilterChipSet` in the `unnamed` slot of `BottomSheet`. -}
filterChipSet :
    M3e.Build.FilterChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
filterChipSet =
    unnamed_core


{-| Place a `FilterChip` in the `unnamed` slot of `BottomSheet`. -}
filterChip :
    M3e.Build.FilterChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
filterChip =
    unnamed_core


{-| Place a `ChipSet` in the `unnamed` slot of `BottomSheet`. -}
chipSet :
    M3e.Build.ChipSet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
chipSet =
    unnamed_core


{-| Place a `AssistChip` in the `unnamed` slot of `BottomSheet`. -}
assistChip :
    M3e.Build.AssistChip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
assistChip =
    unnamed_core


{-| Place a `Chip` in the `unnamed` slot of `BottomSheet`. -}
chip :
    M3e.Build.Chip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
chip =
    unnamed_core


{-| Place a `Checkbox` in the `unnamed` slot of `BottomSheet`. -}
checkbox :
    M3e.Build.Checkbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
checkbox =
    unnamed_core


{-| Place a `Card` in the `unnamed` slot of `BottomSheet`. -}
card :
    M3e.Build.Card.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
card =
    unnamed_core


{-| Place a `Calendar` in the `unnamed` slot of `BottomSheet`. -}
calendar :
    M3e.Build.Calendar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
calendar =
    unnamed_core


{-| Place a `YearView` in the `unnamed` slot of `BottomSheet`. -}
yearView :
    M3e.Build.YearView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
yearView =
    unnamed_core


{-| Place a `MultiYearView` in the `unnamed` slot of `BottomSheet`. -}
multiYearView :
    M3e.Build.MultiYearView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
multiYearView =
    unnamed_core


{-| Place a `MonthView` in the `unnamed` slot of `BottomSheet`. -}
monthView :
    M3e.Build.MonthView.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
monthView =
    unnamed_core


{-| Place a `Tooltip` in the `unnamed` slot of `BottomSheet`. -}
tooltip :
    M3e.Build.Tooltip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tooltip =
    unnamed_core


{-| Place a `RichTooltip` in the `unnamed` slot of `BottomSheet`. -}
richTooltip :
    M3e.Build.RichTooltip.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
richTooltip =
    unnamed_core


{-| Place a `TooltipElementBase` in the `unnamed` slot of `BottomSheet`. -}
tooltipElementBase :
    M3e.Build.TooltipElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
tooltipElementBase =
    unnamed_core


{-| Place a `RichTooltipAction` in the `unnamed` slot of `BottomSheet`. -}
richTooltipAction :
    M3e.Build.RichTooltipAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
richTooltipAction =
    unnamed_core


{-| Place a `ButtonGroup` in the `unnamed` slot of `BottomSheet`. -}
buttonGroup :
    M3e.Build.ButtonGroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
buttonGroup =
    unnamed_core


{-| Place a `IconButton` in the `unnamed` slot of `BottomSheet`. -}
iconButton :
    M3e.Build.IconButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
iconButton =
    unnamed_core


{-| Place a `Button` in the `unnamed` slot of `BottomSheet`. -}
button :
    M3e.Build.Button.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
button =
    unnamed_core


{-| Place a `Breadcrumb` in the `unnamed` slot of `BottomSheet`. -}
breadcrumb :
    M3e.Build.Breadcrumb.Builder ca { cs
        | unnamed : M3e.Build.Internal.Filled
    } msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
breadcrumb =
    unnamed_core


{-| Place a `BreadcrumbItem` in the `unnamed` slot of `BottomSheet`. -}
breadcrumbItem :
    M3e.Build.BreadcrumbItem.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
breadcrumbItem =
    unnamed_core


{-| Place a `BreadcrumbItemButton` in the `unnamed` slot of `BottomSheet`. -}
breadcrumbItemButton :
    M3e.Build.BreadcrumbItemButton.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
breadcrumbItemButton =
    unnamed_core


{-| Place a `BottomSheetTrigger` in the `unnamed` slot of `BottomSheet`. -}
bottomSheetTrigger :
    M3e.Build.BottomSheetTrigger.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
bottomSheetTrigger =
    unnamed_core


{-| Place a `BottomSheet` in the `unnamed` slot of `BottomSheet`. -}
bottomSheet :
    M3e.Build.BottomSheet.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
bottomSheet =
    unnamed_core


{-| Place a `BottomSheetAction` in the `unnamed` slot of `BottomSheet`. -}
bottomSheetAction :
    M3e.Build.BottomSheetAction.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
bottomSheetAction =
    unnamed_core


{-| Place a `Badge` in the `unnamed` slot of `BottomSheet`. -}
badge :
    M3e.Build.Badge.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
badge =
    unnamed_core


{-| Place a `Avatar` in the `unnamed` slot of `BottomSheet`. -}
avatar :
    M3e.Build.Avatar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
avatar =
    unnamed_core


{-| Place a `Autocomplete` in the `unnamed` slot of `BottomSheet`. -}
autocomplete :
    M3e.Build.Autocomplete.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
autocomplete =
    unnamed_core


{-| Place a `FormField` in the `unnamed` slot of `BottomSheet`. -}
formField :
    M3e.Build.FormField.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
formField =
    unnamed_core


{-| Place a `OptionPanel` in the `unnamed` slot of `BottomSheet`. -}
optionPanel :
    M3e.Build.OptionPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
optionPanel =
    unnamed_core


{-| Place a `FloatingPanel` in the `unnamed` slot of `BottomSheet`. -}
floatingPanel :
    M3e.Build.FloatingPanel.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
floatingPanel =
    unnamed_core


{-| Place a `Optgroup` in the `unnamed` slot of `BottomSheet`. -}
optgroup :
    M3e.Build.Optgroup.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
optgroup =
    unnamed_core


{-| Place a `Option` in the `unnamed` slot of `BottomSheet`. -}
option :
    M3e.Build.Option.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
option =
    unnamed_core


{-| Place a `FocusTrap` in the `unnamed` slot of `BottomSheet`. -}
focusTrap :
    M3e.Build.FocusTrap.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
focusTrap =
    unnamed_core


{-| Place a `AppBar` in the `unnamed` slot of `BottomSheet`. -}
appBar :
    M3e.Build.AppBar.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
appBar =
    unnamed_core


{-| Place a `TextOverflow` in the `unnamed` slot of `BottomSheet`. -}
textOverflow :
    M3e.Build.TextOverflow.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
textOverflow =
    unnamed_core


{-| Place a `TextHighlight` in the `unnamed` slot of `BottomSheet`. -}
textHighlight :
    M3e.Build.TextHighlight.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
textHighlight =
    unnamed_core


{-| Place a `StateLayer` in the `unnamed` slot of `BottomSheet`. -}
stateLayer :
    M3e.Build.StateLayer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
stateLayer =
    unnamed_core


{-| Place a `Slide` in the `unnamed` slot of `BottomSheet`. -}
slide :
    M3e.Build.Slide.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
slide =
    unnamed_core


{-| Place a `ScrollContainer` in the `unnamed` slot of `BottomSheet`. -}
scrollContainer :
    M3e.Build.ScrollContainer.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
scrollContainer =
    unnamed_core


{-| Place a `Ripple` in the `unnamed` slot of `BottomSheet`. -}
ripple :
    M3e.Build.Ripple.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
ripple =
    unnamed_core


{-| Place a `PseudoRadio` in the `unnamed` slot of `BottomSheet`. -}
pseudoRadio :
    M3e.Build.PseudoRadio.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
pseudoRadio =
    unnamed_core


{-| Place a `PseudoCheckbox` in the `unnamed` slot of `BottomSheet`. -}
pseudoCheckbox :
    M3e.Build.PseudoCheckbox.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
pseudoCheckbox =
    unnamed_core


{-| Place a `FocusRing` in the `unnamed` slot of `BottomSheet`. -}
focusRing :
    M3e.Build.FocusRing.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
focusRing =
    unnamed_core


{-| Place a `Elevation` in the `unnamed` slot of `BottomSheet`. -}
elevation :
    M3e.Build.Elevation.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
elevation =
    unnamed_core


{-| Place a `Collapsible` in the `unnamed` slot of `BottomSheet`. -}
collapsible :
    M3e.Build.Collapsible.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
collapsible =
    unnamed_core


{-| Place a `ActionElementBase` in the `unnamed` slot of `BottomSheet`. -}
actionElementBase :
    M3e.Build.ActionElementBase.Builder ca cs msg ck
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
    -> M3e.Build.BottomSheet.Builder pa ps msg pk
actionElementBase =
    unnamed_core