...
primitive: a form-field's control-slot setter stamps `id="<id>"`
and its label-slot setter stamps `for="<id>"`, so the label↔control association
is structural. Decomposes the retired `Content.slotWithAttr`:
...
the result (a crossing, via [`fromNode`](#fromNode)). This is the named-slot
placement primitive: the generated per-slot setters constrain their
