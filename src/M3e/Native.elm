module M3e.Native exposing
    ( a, label, input, textarea, select, button, span, div, p, ul, li, nav, header, footer, section, img
    , alt, checked, cols, disabled, download, for, height, href, max, min, multiple, name, placeholder, readonly, rel, required, rows, src, step, target, type_, value, width
    )

{-| Typed native HTML: element constructors (`div`, `a`, `input`, …) plus
attribute setters that only accept what HTML permits on that element. Each
constructor's attribute record is closed to that element's real attributes
(plus `slot`), so `M3e.Native.div [ M3e.Native.href "x" ] []` is a compile
error — a `div` has no `href`. Semantic tags like `a` and `label` carry their
kind, so they fit the slots that expect a link or a label.
...
-}
a : List (Attr { download : Supported, href : Supported, rel : Supported, slot : Supported, target : Supported } msg) -> List (Element s msg) -> Element { k | link : Supported } msg
a attrs children =
    Element.fromNode
        (Node.fromComponent
...
-}
label : List (Attr { for : Supported, slot : Supported } msg) -> List (Element s msg) -> Element { k | label : Supported } msg
...
-}
input : List (Attr { checked : Supported, disabled : Supported, max : Supported, min : Supported, multiple : Supported, name : Supported, placeholder : Supported, readonly : Supported, required : Supported, slot : Supported, step : Supported, type_ : Supported, value : Supported } msg) -> Element { k | html : Supported } msg
input attrs =
    Element.fromNode
        (Node.fromComponent
...
-}
textarea : List (Attr { cols : Supported, disabled : Supported, name : Supported, placeholder : Supported, readonly : Supported, required : Supported, rows : Supported, slot : Supported, value : Supported } msg) -> List (Element s msg) -> Element { k | html : Supported } msg
...
-}
select : List (Attr { disabled : Supported, multiple : Supported, name : Supported, required : Supported, slot : Supported } msg) -> List (Element s msg) -> Element { k | html : Supported } msg
...
-}
button : List (Attr { disabled : Supported, name : Supported, slot : Supported, type_ : Supported, value : Supported } msg) -> List (Element s msg) -> Element { k | html : Supported } msg
...
-}
img : List (Attr { alt : Supported, height : Supported, slot : Supported, src : Supported, width : Supported } msg) -> Element { k | html : Supported } msg
img attrs =
    Element.fromNode
        (Node.fromComponent
