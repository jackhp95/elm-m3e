...
shape attributes children =
    M3e.Raw.Shape.shape (List.map M3e.Html.Attr.toAttribute attributes) children
...
name :
    ...
    -> M3e.Html.Attr.Attr { c | nameEnum : M3e.Token.Supported } msg
name v_ =
    M3e.Html.Attr.Internal.attribute M3e.Raw.Shape.name (M3e.Token.toString v_)
