module M3e.Raw.FabMenuItem exposing
    ( fabMenuItem, disabled, download, href, rel, target
    , onClick
    )

{-| Bottom layer for `<m3e-fab-menu-item>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
