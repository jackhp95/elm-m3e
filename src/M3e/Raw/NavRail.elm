module M3e.Raw.NavRail exposing (navRail, mode, onBeforeinput, onInput, onChange)

{-| Bottom layer for `<m3e-nav-rail>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
