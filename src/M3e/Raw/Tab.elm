module M3e.Raw.Tab exposing
    ( tab, disabled, for, selected, onBeforeinput, onInput
    , onChange, onClick
    )

{-| Bottom layer for `<m3e-tab>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
