module M3e.Raw.Icon exposing
    ( icon, filled, grade, opticalSize, name, variant
    , weight
    )

{-| Bottom layer for `<m3e-icon>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
