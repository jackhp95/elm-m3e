module M3e.Raw.Autocomplete exposing
...
{-| Bottom layer for `<m3e-autocomplete>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
autocomplete : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
autocomplete =
