module M3e.Raw.Tooltip exposing
    ( tooltip, disabled, for, hideDelay, position, showDelay
    , touchGestures
    )

{-| Bottom layer for `<m3e-tooltip>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
