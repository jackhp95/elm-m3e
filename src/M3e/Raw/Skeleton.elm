module M3e.Raw.Skeleton exposing (skeleton, animation, shape, loaded)

{-| Bottom layer for `<m3e-skeleton>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
