module M3e.Raw.BreadcrumbItem exposing
    ( breadcrumbItem, itemLabel, disabled, current, href, target
    , download, rel, onClick
    )

{-| Bottom layer for `<m3e-breadcrumb-item>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
breadcrumbItem : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
breadcrumbItem =
