module M3e.Raw.ButtonGroup exposing (buttonGroup, multi, size, variant)

{-| Bottom layer for `<m3e-button-group>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
