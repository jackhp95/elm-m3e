module M3e.Raw.BreadcrumbItemButton exposing
    ( breadcrumbItemButton, current, href, target, rel, download
    , disabled, onClick
    )

{-| Bottom layer for `<m3e-breadcrumb-item-button>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
breadcrumbItemButton : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
