module M3e.Raw.Switch exposing
    ( switch, checked, disabled, icons, name, value
    , onBeforeinput, onInput, onChange, onClick
    )

{-| Bottom layer for `<m3e-switch>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
