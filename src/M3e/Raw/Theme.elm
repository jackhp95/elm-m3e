module M3e.Raw.Theme exposing
    ( theme, color, contrast, density, scheme, strongFocus
    , variant, motion, onChange
    )

{-| Bottom layer for `<m3e-theme>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
