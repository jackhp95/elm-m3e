module M3e.Raw.Collapsible exposing
    ( collapsible, open, orientation, noAnimate, onOpening, onOpened
    , onClosing, onClosed
    )

{-| Bottom layer for `<m3e-collapsible>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
