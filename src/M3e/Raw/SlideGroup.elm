module M3e.Raw.SlideGroup exposing (slideGroup, disabled, nextPageLabel, previousPageLabel, threshold, vertical)

{-| Bottom layer for `<m3e-slide-group>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
