module M3e.Raw.SegmentedButton exposing
    ( segmentedButton, disabled, hideSelectionIndicator, multi, name, onChange
    , onBeforeinput, onInput
    )

{-| Bottom layer for `<m3e-segmented-button>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
segmentedButton : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
segmentedButton =
