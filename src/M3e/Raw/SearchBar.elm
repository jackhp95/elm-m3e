module M3e.Raw.SearchBar exposing (searchBar, clearable, clearLabel, onClear)

{-| Bottom layer for `<m3e-search-bar>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
