module M3e.Raw.InputChipSet exposing (inputChipSet, disabled, name, required, vertical, onChange)

{-| Bottom layer for `<m3e-input-chip-set>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
inputChipSet : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
