module M3e.Raw.TextHighlight exposing (textHighlight, caseSensitive, disabled, mode, term, onHighlight)

{-| Bottom layer for `<m3e-text-highlight>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
textHighlight : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
textHighlight =
