module M3e.Raw.ExpansionHeader exposing (expansionHeader, hideToggle, toggleDirection, togglePosition, disabled, onClick)

{-| Bottom layer for `<m3e-expansion-header>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
expansionHeader : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
expansionHeader =
