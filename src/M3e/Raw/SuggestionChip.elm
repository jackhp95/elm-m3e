module M3e.Raw.SuggestionChip exposing
    ( suggestionChip, disabled, disabledInteractive, download, href, name
    , rel, target, type_, value, variant, onClick
    )

{-| Bottom layer for `<m3e-suggestion-chip>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
suggestionChip : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
suggestionChip =
