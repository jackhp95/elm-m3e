module M3e.Raw.ButtonSegment exposing
    ( buttonSegment, checked, disabled, value, onBeforeinput, onInput
    , onChange, onClick
    )

{-| Bottom layer for `<m3e-button-segment>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
buttonSegment : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
buttonSegment =
