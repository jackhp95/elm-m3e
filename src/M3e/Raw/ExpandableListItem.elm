module M3e.Raw.ExpandableListItem exposing
    ( expandableListItem, disabled, open, onOpening, onOpened, onClosing
    , onClosed
    )

{-| Bottom layer for `<m3e-expandable-list-item>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
expandableListItem : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
expandableListItem =
