module M3e.Raw.Badge exposing (badge, size, position, for)

{-| Bottom layer for `<m3e-badge>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
