module M3e.Raw.ExpansionPanel exposing
    ( expansionPanel, disabled, hideToggle, open, toggleDirection, togglePosition
    , onOpening, onOpened, onClosing, onClosed
    )

{-| Bottom layer for `<m3e-expansion-panel>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
expansionPanel : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
expansionPanel =
