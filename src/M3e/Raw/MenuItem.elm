module M3e.Raw.MenuItem exposing
    ( menuItem, disabled, download, href, rel, target
    , onClick
    )

{-| Bottom layer for `<m3e-menu-item>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
