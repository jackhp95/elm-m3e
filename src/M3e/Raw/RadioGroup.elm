module M3e.Raw.RadioGroup exposing
    ( radioGroup, ariaInvalid, disabled, name, required, onBeforeinput
    , onInput, onChange
    )

{-| Bottom layer for `<m3e-radio-group>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
