module M3e.Raw.ListItemButton exposing
    ( listItemButton, href, target, rel, download, disabled
    , onClick
    )

{-| Bottom layer for `<m3e-list-item-button>`: the plain `elm/html` API — one element constructor plus raw attribute and event setters, faithful DOM emission, no phantom typing. The rawest escape in the gradient.
...
-}
listItemButton : List (Html.Attribute msg) -> List (Html.Html msg) -> Html.Html msg
listItemButton =
