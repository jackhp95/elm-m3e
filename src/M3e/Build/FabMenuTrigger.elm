module M3e.Build.FabMenuTrigger exposing
    ( Builder, AttrCaps, SlotCaps, fabMenuTrigger, attr, for
    , child, build
    )

{-| The Build form for `<m3e-fab-menu-trigger>` — phantom-typed pipeline API. Import qualified: `import M3e.Build.FabMenuTrigger as FabMenuTrigger`.

@docs Builder, AttrCaps, SlotCaps, fabMenuTrigger, attr, for
@docs child, build
...
for v_ b_ =
    M3e.Build.Internal.wrap_
        (Markup.Node.addAttr
            (Markup.Html.Attr.Internal.forget (M3e.Html.FabMenuTrigger.for v_))
            (M3e.Build.Internal.node_ b_)
        )
...
build :
    Builder a s msg kind
    -> Markup.Element.Element { fabMenuTrigger : M3e.Kind.Brand } msg
build b_ =
