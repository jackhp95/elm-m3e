...
type alias Builder attrCaps slotCaps msg kind =
    M3e.Build.Internal.Builder
        { kind
            | expansionPanel : M3e.Token.Supported
        }
        attrCaps
        slotCaps
        msg
...
open v_ b_ =
    M3e.Build.Internal.wrap_
        (M3e.Node.addAttr
            (M3e.Html.Attr.Internal.forget (M3e.Html.ExpansionPanel.open v_))
            (M3e.Build.Internal.node_ b_)
        )
...
build :
    Builder a s msg kind
    -> M3e.Element.Element { expansionPanel : M3e.Token.Supported } msg
build b_ =
