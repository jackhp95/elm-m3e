...
description: >-
  ...
  types. Covers the five-surface decision tree (M3e barrel / per-component / M3e.Record /
...
hand-edit it. The library ships **no runtime beyond `elm/html`** — an `M3e.button` is a
lazy IR node that becomes a `<m3e-button>` only at `Element.toNode`.

## The #1 trap: register the custom elements

`elm install jackhp95/elm-m3e` gives you the Elm side only. The `<m3e-*>` custom
elements are inert HTML until the matching JS is registered at app startup. **If you
skip this, every component renders as an empty/unstyled box and nothing is wrong with
your Elm.** Wire it once:
...
## The five surfaces — decision tree

Two orthogonal axes shape the API (see `docs/DESIGN.md` §1, §3). The **layer** axis is a
safety gradient; the **form** axis picks how required vs optional parts are passed. All
surfaces return the same `M3e.Element.Element … msg`, so they nest and interchange
freely. Default to Standard; move only when a concrete guarantee earns it.

| Want | Use | Why |
|------|-----|-----|
| The everyday default | **`M3e` barrel** (form 3, standard) — `M3e.button [attrs] [content]` | one import (`import M3e exposing (..)`), lowercase names, generalized setters. Start here. |
| A component with **required** content (label, action) checked by the compiler | **`M3e.Record.*`** (form 4) — `view { required } [attrs] [content]` | missing-required becomes a *compile* error, not a lint. Only components that have required parts get a Record module. |
| A pipeline where **duplicate-singular is unwritable** | **`M3e.Build.*`** (form 5) — `seed {req} \|> setter \|> build`, imported qualified | setting a singular attr twice fails to type-check. Opt-in, per-component, no barrel. |
| A raw attribute the typed layer doesn't surface | **`M3e.Html.*`** (middle) — lazy IR attrs, eager component | documented escape, less convenience. |
| Partial applications of `elm/html`, no phantom types | **`M3e.Raw.*`** (bottom) — fully applied → `Html` | the documented floor; the last escape hatch. |

`M3e.Html`/`M3e.Raw` are **documented escapes**, not the everyday path. Reach for them
only when the typed surface genuinely can't express something; prefer filing the gap.

**Rule of thumb:** build a settings screen or a form → **Standard** (`M3e` barrel).
A button/dialog/snackbar that *must* carry a label or action → **Record**. A guided,
...
## The seam: text and link are userland

The library intentionally does **not** define `text`/`link`. Slotted text needs a wrapper
and navigation is a plain `<a>`, but *what* your text is (a `String`? an i18n key?) is a
team decision. These live in the **Kit**, a copyable module set under `docs/kit/` that is
**NOT part of the published package** — fork consumers copy `Kit`, `Native`, and `Seam`
into their own app and adapt them (see `docs/kit/README.md`, `docs/DESIGN.md` §4):

- **`Kit`** — `text` / `link` producers carrying the `text` / `link` kind.
- **`Native`** — native-HTML IR (`div`, `span`, `p`, `a`, `img`, `ul`, `li`, …) carrying
  the `html` kind, so it fits any `"arbitrary"` slot.
- **`Seam`** — the single sanctioned, loud, auditable break-glass (`fromHtml`, `recast`)
  ...
  else; that fence is Elm's own module exposure, not a lint rule.

So a real call reads `M3e.button [ M3e.variant M3e.Token.filled ] [ Kit.text "Save" ]` —
library component, userland text.
...
```elm
Element.toNode myScreen   -- IR → Node → Html, the only eager point
```

## Reference

- **[reference/surfaces-and-starter.md](reference/surfaces-and-starter.md)** — the
  full surface-comparison table, the mistake→enforcement matrix, and a compilable
