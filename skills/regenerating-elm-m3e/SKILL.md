...
(The hand-written IR *core* — `src/M3e/Node.elm`, `src/M3e/Element.elm` — is the
exception; those are the only elm-format-validated files in CI. Per-component modules are
deliberately excluded from `elm-format --validate` because they are generated.)
...
|------------|----------------------------------|
| `slots.<name>.kinds` | the slot's phantom kind row (closed set, `"arbitrary"` open row, or `[]`) |
...
- Any change to `src/M3e/Node.elm`/`Element.elm` from a regen — those are hand-written;
  the generator should not touch them.
