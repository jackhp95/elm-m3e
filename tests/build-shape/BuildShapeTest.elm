...
selectWithOption =
    Select.select
        |> SelectSlots.option (Option.option { content = Markup.Atoms.text "Option A" })
        |> SelectSlots.option (Option.option { content = Markup.Atoms.text "Option B" })
        |> Select.build
...
radioGroupWithFilledSelect =
    RadioGroup.radioGroup
        |> RadioGroupSlots.unnamed
            (Select.select
                |> SelectSlots.option (Option.option { content = Markup.Atoms.text "Opt" })
            )
        |> RadioGroup.build
...
1.  — no `slotDefault`/`child` wrapper.
