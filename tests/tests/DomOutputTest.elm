...
compositions through the single eager exit (`Element.toNode` then `Node.toHtml`)
and assert the shape of the emitted `Html`. Where `IrCoreTest`/`NodeSlotTest`
probe the hand-written IR-core reductions in isolation, this suite pins the
*public* contract a screen author sees: the right `<m3e-*>` custom-element tag,
...
into a query. This is the only eager point, mirroring `Element.toNode` at a
`view` root. Rendered as the query root so the top-level `<m3e-*>` tag is
directly assertable.
-}
toQuery : Element.Element supported msg -> Query.Single msg
toQuery el =
    Element.toNode el
        |> Node.toHtml
        |> Query.fromHtml
    ...
    describe "DOM-output shape of representative compositions"
        ...
        , describe "attribute and token emission"
            [ test "a variant token becomes a variant= attribute on the host" <|
                \_ ->
                    M3e.button [ M3e.variantFilled ] [ Kit.text "Go" ]
                        |> toQuery
                        |> Query.has
            ...
            , test "an aria-label setter becomes an aria-label attribute" <|
                \_ ->
                    M3e.iconButton [ M3e.ariaLabel "Close" ]
                        [ M3e.icon [ M3e.attrName "close" ] [] ]
                        |> toQuery
                        |> Query.has
        ...
        , describe "slot stamping on slotted children"
            [ test "buttonSlotIcon stamps slot=icon on the placed icon" <|
                \_ ->
                    M3e.button []
            ...
            , test "listItemSlotLeading stamps slot=leading on the placed content" <|
                \_ ->
                    M3e.listItem []
                        [ M3e.listItemSlotLeading (M3e.icon [ M3e.attrName "star" ] [])
                        , Kit.text "Starred"
                        ]
                        |> toQuery
