...
pageHeading =
    M3e.heading { content = M3e.text "Examples" }
...
exampleCard ( slug, cardTitle, cardBody ) =
    ...
    M3e.card
        ...
        , M3e.Component.Card.actions
            (M3e.button { content = M3e.text ("Open " ++ cardTitle), action = M3e.Action.none } [ M3e.Component.Button.variant Value.filled, M3e.Component.Button.href exampleHref ] [])
        ]
