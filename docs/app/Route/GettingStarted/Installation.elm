...
"""
                    , TypedHtml.p [ TA.class "text-body-lg text-on-surface-variant" ]
                        [ M3e.text "HtmlIr.Element.toNode turns an M3e Element into elm/html, so Browser.sandbox can render it (add `import Html` alongside the imports above). Finally, an index.html loads the CSS, registers the components, and boots Elm:" ]
                    , code_ Xml """
<!doctype html>
<html lang="en">
  <head>
