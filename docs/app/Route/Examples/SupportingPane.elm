...
-}
screen : Element { s | html : Supported, link : Supported } msg
screen =
...
crowds the primary.
-}
body : Element { s | html : Supported } msg
body =
...
-}
primary : Element { s | html : Supported } msg
primary =
...
summaryCard : Element { s | card : Supported } msg
summaryCard =
...
metric : String -> String -> Element { s | html : Supported } msg
metric value label =
...
milestonesCard : Element { s | card : Supported } msg
milestonesCard =
...
milestoneRow : String -> String -> String -> Element { s | listItem : Supported } msg
...
-}
supporting : Element { s | html : Supported } msg
supporting =
...
activityRow : Activity -> Element { s | html : Supported } msg
activityRow a =
...
tag : String -> Element { s | assistChip : Supported } msg
tag label =
...
desktopRail : Element { s | navRail : Supported } msg
desktopRail =
    M3e.navRail [ Layout.class "hidden md:flex shrink-0" ]
        (List.map navItem destinations)


mobileBar : Element { s | navBar : Supported } msg
mobileBar =
    M3e.navBar [ Layout.class "md:hidden fixed inset-x-0 bottom-0" ]
        (List.map navItem destinations)


navItem : { icon : String, label : String } -> Element { s | navItem : Supported } msg
navItem d =
    M3e.navItem
