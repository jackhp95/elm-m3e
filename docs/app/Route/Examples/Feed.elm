...
-}
filterBar : String -> Element { s | filterChipSet : Supported } Msg
filterBar current =
    M3e.filterChipSet []
        (List.map (filterChip current) filters)


filterChip : String -> String -> Element { s | filterChip : Supported } Msg
...
-}
cardGrid : List Post -> Element { s | html : Supported } msg
cardGrid shown =
    Layout.div "grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3"
        (List.map postCard shown)


postCard : Post -> Element { s | card : Supported } msg
postCard post =
