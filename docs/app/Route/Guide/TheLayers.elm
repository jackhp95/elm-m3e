...
handful of interchangeable **surfaces** (barrel, the standard constructor, `required`, `build`), plus a
...
layersDiagram =
  ...
  M3e.Component.Button.required { … } …            required-record form (the 29 with a required record)
...
recap : String
recap =
    """- A component is **one typed value**, written through interchangeable **surfaces** (barrel, standard constructor, `required`, `build`) — **peers, not a ranking**.
