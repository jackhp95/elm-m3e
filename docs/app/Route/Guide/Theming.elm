...
rootBody : String
rootBody =
    """Wrap the app — or any subtree — in `M3e.Theme.el`. It is a non-visual element: it emits no box of its own, it just publishes the derived token roles to everything nested inside it. The docs app themes once, at the root, in `docs/app/Shared.elm`:"""
...
recap : String
recap =
    """- Theme **once, at the root**: `M3e.Theme.el` fed a **seed color plus scheme / contrast / density** derives every `--md-sys-*` role.
