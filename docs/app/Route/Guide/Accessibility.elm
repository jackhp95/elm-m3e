...
-}
labeledBack : Element { s | iconButton : Supported } msg
labeledBack =
...
nameSurfaces : String
nameSurfaces =
    """The ARIA setter exists on three surfaces; pick the one matching your build layer: the barrel `M3e.ariaLabel "Back"` / `M3e.ariaLabelledby "…"`, the mid layer `M3e.Aria.label "Back"` / `M3e.Aria.labelledby "…"`, or the bottom `M3e.Raw.Aria.label "Back"`. They are first-class on *every* component, right where you reach for the other attributes — accessibility is not a separate module you have to remember to import. The trailing `Switch`/`Radio` in the [Settings example](/examples/settings) pass `M3e.ariaLabel label` on every control for exactly the sibling-label reason above."""
