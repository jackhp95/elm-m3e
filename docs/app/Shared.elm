...
appShellBar =
    Html.header
        ...
        [ AppBar.view
            ...
            , AppBar.trailing (Seam.recast githubLink)
            , AppBar.trailing (Seam.recast settingsButton)
            ]
            |> toHtml
        ]
...
-}
settingsDrawerContent : Model -> Element { s | html : Supported } Msg
...
controlLabel : String -> Element { s | html : Supported } Msg
controlLabel label =
...
-}
segmented : List ( String, Bool, Msg ) -> Element { s | segmentedButton : Supported } Msg
segmented segments =
...
schemeSegmented : Model -> Element { s | segmentedButton : Supported } Msg
schemeSegmented model =
    segmented
...
contrastSegmented : Model -> Element { s | segmentedButton : Supported } Msg
contrastSegmented model =
    segmented
...
-}
seedColorInput : Model -> Element { s | formField : Supported } Msg
seedColorInput model =
    FormField.view [ FormField.variant Value.outlined ]
        [ FormField.label "seed-color" (Native.label [] [ Kit.text "Source color" ])
        , FormField.hint (Kit.code Value.small [ Kit.onSurfaceVariant ] [ Kit.text model.seed ])
        , FormField.control "seed-color"
            (M3e.Native.input
...
densitySegmented : Model -> Element { s | segmentedButton : Supported } Msg
densitySegmented model =
    segmented
...
directionSegmented : Model -> Element { s | segmentedButton : Supported } Msg
directionSegmented model =
    segmented
...
-}
navMenu : List NavComponent -> String -> Element { s | navMenu : Supported } msg
navMenu components currentPath =
    NavMenu.view []
...
category groups (#106).
-}
componentsGroup : List NavComponent -> String -> Element { s | navMenuItem : Supported } msg
componentsGroup components currentPath =
    NavMenuItem.view
