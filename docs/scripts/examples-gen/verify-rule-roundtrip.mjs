...
function main() {
  ...
  const notes = [
    ...
    "slot-vocabulary-asymmetry (ALLOWLISTED): a barrelised slot round-trips to a component-specific barrel slot name (matching `M3e.…Slot…` / `M3e.…SlotDefault`) rather than back to the per-component setter. The endpoints coincide in type — the barrel slot name is the per-component setter re-exported under a flat name (same phantom row, same output) — so only the literal name differs. PreferSpecificSlot's slot-upgrade class targets that specific barrel name. Everything else round-trips exactly.",
