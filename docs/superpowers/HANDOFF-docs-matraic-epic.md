...
**Date:** 2026-07-02 · **Status:** Plans 1 & 2 LANDED; Plan 3 FIRST GREEN INCREMENT LANDED (B1 + A2 + A3 + B3 + B4 + B2, all pushed & browser-validated). **Only Phase A1 (mid/bottom converter layers) remains** — see §3.

> **UPDATE 2026-07-02 (increment 2):** The whole shippable increment below (§3 "REMAINING") is DONE and pushed. Component pages now render live Usage sections (raw-html preview + code), the elm-review gate is GREEN, the drawer nav is categorized (7 groups), and a 2-option API-layer toggle (M3e Elm / raw HTML) works. Validated in Chromium: 125/125 pages mount clean + `docs/tests-browser/{usage,nav}.spec.ts`. `docs/data/examples.json` + `config/examples.rich.json` are generated by `build:assets`. **The ONLY remaining Plan-3 work is Phase A1** (converter emits `M3e.Cem.*` middle + `M3e.Cem.Html.*` bottom Elm, compile-verified per layer → populate `mid`/`bottom` → expand the toggle to 4 options). Everything below §3 predates this and is kept for reference.
