...
- **Typed native-HTML IR** (`M3e.Native`, `M3e.Content`) with HTML-natural
...
- **A structural opaque-IR boundary.** `M3e.Node` / `M3e.Element` are opaque lazy
  ...
  `M3e.*` code and a team's designated `Seam` module (ADR 0014).
...
- The `packages/m3e-kit/` **Kit** (`Kit`, `Native`, `Seam`, `Layout`) is
