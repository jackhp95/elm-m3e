...
codegenAware =
    let
        ...
        -- exempts what the facts-derived correctness rules do not: the docs app's
        -- own adapters (`Doc`, `Shared`), which deliberately hold the tight
        -- component-module form, so barrelising them is wrong by design. The
        -- `Layout`/`Kit`/`Native`/`Seam` adapters this list used to name are gone —
        -- the library's generated surface replaced them. Every other codegen-aware
        -- rule stays on everywhere: they enforce facts-derived correctness that
        -- holds on every layer/form. Two path forms per entry cover root- vs
        -- `docs/`-relative review runs.
